...
const LlmReply: React.FC<LlmReplyProps> = ({ chat, userQuestion }) => {
  ...
  function renderButtons(): React.ReactNode {
    return (
      <div className="button-group">
        <button onClick={() => setShowAnaliticsSection(!showAnaliticsSection)}>
          {showAnaliticsSection ? "Hide Analytics" : "Show Analytics"}
        </button>
        <button onClick={() => setShowQueryEditSection(!showQueryEditSection)}>
          {showQueryEditSection ? "Hide Query Editor" : "Edit Query"}
        </button>
  ...
  function renderMessageContent(): React.ReactNode {
    if (isSingleResponse) {
      return <div>{String(message.result)}</div>;
    } else if (Array.isArray(message.result)) {
      return <Table data={message.result} loadingUi={loadingUi} chatId={id} />;
    } else {
