...
const Chat: React.FC = () => {
  ...
  const [codeLangFrom, setCodeLangFrom] = React.useState("");
  ...
  const processFiles = async (transform_to: string) => {
    ...
    fetch(TRANSFORM_CODE, requestOptions)
  ...
  const handleChangeFrom = async (event: SelectChangeEvent) => {
    setCodeLangFrom(event.target.value as string);
  };

  const handleChange = async (event: SelectChangeEvent) => {
    setCodeLang(event.target.value as string);
    await processFiles(event.target.value as string);
  };
      ...
      <div className="chat-hldr">
        <div className="chat-scrollhldr">
          ...
          <div className="chat-msg">
            ...
            {repoFiles && (
              <UserChat chat={{ message: "Rewriting and convert the code" }} />
            )}
            {showDropdown && (
              <>
                <LlmReply
                  chat={{
                    message:
                      "Please select the source language of the code you would like to convert.",
                  }}
                />
                ...
                <div className="chat-msg-list msg-hldr-usr">
                  ...
                  >
                    ...
                    <Select
                      ...
                      disabled={!!codeLangFrom}
                    >
                      <MenuItem value={".jsx"}>JavaScript</MenuItem>
                      <MenuItem value={".jsx"}>React JS</MenuItem>
                      <MenuItem value={".py"}>Python</MenuItem>
                      <MenuItem value={".html"}>HTML</MenuItem>
                      <MenuItem value={".java"}>Java</MenuItem>
                      <MenuItem value={".cbl"}>COBOL</MenuItem>
                    </Select>
                  </div>
                </div>
              </>
            ...
            {codeLangFrom && (
              <>
                <LlmReply
                  chat={{
                    message: `Sure! All files with the "${codeLangFrom}" extension will be converted. Please select the target language.`,
                  }}
                />
            ...
            {/* codeProcess */}
            {loading && <Loader text="Fetching code from repo" />}
            {loadingProcess && (
              <Loader
                text={`Converting all the ${codeLangFrom} files to ${codeLang}`}
              />
            )}
          ...
          >
            <div className="Input-Container">
              <input
                ...
                id="query"
                name="query"
                value={"https://github.com/Biswanathdas1996/code-review"}
              />
